...
const SuperAdmin = () => {
  ...
  const { data: todaysShifts } = useQuery({
    queryKey: ['superadmin-todays-shifts'],
    queryFn: async () => {
  ...
  const { data: weeklyShifts } = useQuery({
    queryKey: ['superadmin-weekly-shifts'],
    queryFn: async () => {
      const today = new Date();
  ...
  const handleSaveShift = async (
    ...
    try {
      ...
      queryClient.invalidateQueries({ queryKey: ["superadmin-todays-shifts"] });
      queryClient.invalidateQueries({ queryKey: ["superadmin-weekly-shifts"] });
    } catch (error) {
      ...
      <div className="flex items-center justify-center min-h-screen">
        <Card className="w-96">
          <CardHeader>
            ...
            <CardDescription>
              Du har ikke tillatelse til å få tilgang til superadmin-siden.
            </CardDescription>
          </CardHeader>
        </Card>
      </div>
